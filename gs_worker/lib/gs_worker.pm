...
# via MUPDFAPI or pdftocairo. Returns results as JSON.
...
if ($GS_WORKER_AUTH_SECRET eq '' && ($ENV{GS_WORKER_IS_ISOLATED} // '') ne '1') {
    die "gs_worker: set GS_WORKER_AUTH_SECRET to a shared secret, or set "
      . "GS_WORKER_IS_ISOLATED=1 when the worker runs on an isolated network.\n";
}
my $GS_SCRATCH_DIR        = $ENV{GS_SCRATCH_DIR}        || '/tmp/gs';
my $GS_RENDER_OUTPUT      = lc($ENV{GS_RENDER_OUTPUT}   || 'scratch');
$GS_RENDER_OUTPUT         = 'scratch' unless $GS_RENDER_OUTPUT eq 'stdout';

my $svg_backend = (eval { require MUPDFAPI; 1 }) ? 'mupdf' : 'pdftocairo';
my $mupdf_ctx = $svg_backend eq 'mupdf' ? MUPDFAPI::new_context() : undef;

sub _pdf_to_svg_cairo {
    my ($pdf) = @_;
    my $pid = open2(my $svg_out, my $pdf_in, 'pdftocairo', '-svg', '-', '-');
    binmode $pdf_in;
    binmode $svg_out;
    print {$pdf_in} $pdf;
    close $pdf_in;
    ...
    close $svg_out;
    waitpid $pid, 0;
    die "pdftocairo exited with status " . ($? >> 8) . "\n" if $?;
    return $svg;
}
...
sub _setup_scratch {
    File::Path::make_path($GS_SCRATCH_DIR, { mode => 0700 }) unless -d $GS_SCRATCH_DIR;
    my $now = time();
    for my $f (glob("$GS_SCRATCH_DIR/*")) {
        my $mtime = (stat $f)[9];
        unlink $f if defined $mtime && ($now - $mtime) > 3600;
    }
}
_setup_scratch();
...
sub init_gs {
    ...
    (undef, undef, $gs_revision) = GSAPI::revision();
    info "Render output: $GS_RENDER_OUTPUT; SVG backend: $svg_backend";

    my $fontpath = "${ASSETS_DIR}/fonts";
    my @args = ("-dSAFER", "-dNOEPS", "-I${ASSETS_DIR}", "-I${ASSETS_DIR}/bwipp");
    if ($gs_revision >= 10040) {
        push @args, "--permit-devices=*", "--permit-file-write=${GS_SCRATCH_DIR}/*";
    }
...
sub _run_ps {
    ...
    if ($rc != 0 && $rc <= -100) {
        check_gs_rc($rc);
        my $resp = json_response({rc => $rc, stderr => $gsstderr, error => 'gs_fatal', healthy => \0}, 503);
        return \$resp;
    }
...
post '/gs' => sub {
    ...
    if ($rc != 0) {
        (my $s = $bbox_stderr // '') =~ s/[^[:print:]]/ /g;
        info $L->(sprintf('/gs bbox rc=%d in %.1fms stderr=[%s]', $rc, (time() - $t0) * 1000, substr($s, 0, 500)));
        return json_response({rc => $rc + 0, bbox_error => 'bbox_pserror', stderr => $bbox_stderr}, 200);
    }
    ...
    for my $i (0..$#render_templates) {
        ...
        my ($rrc, $stdout_data, $render_stderr);
        if ($GS_RENDER_OUTPUT eq 'scratch' && $final_ps =~ /\(%stdout\)/) {
            my $is_pdf = $final_ps =~ m{/OutputDevice\s*/pdfwrite};
            my $scratch = File::Temp->new(TEMPLATE => 'gsXXXXXXXX', DIR => $GS_SCRATCH_DIR);
            (my $routed = $final_ps) =~ s/\(%stdout\)/(${\ "$scratch"})/;
            ($rrc, undef, $render_stderr) = _run_ps($L, $routed, 0);
            return $$rrc if ref $rrc;
            $stdout_data = '';
            if ($rrc == 0 && open my $fh, '<:raw', "$scratch") {
                local $/;
                $stdout_data = <$fh>;
                close $fh;
            }
            # Sometimes we have to shoot GS in the head to get it to flush the PDF output stream.
            if ($is_pdf && length($stdout_data) && substr($stdout_data, -64) !~ /%%EOF\s*\z/) {
                debug $L->(sprintf('/gs render %d unfinalised; closing interpreter to finalise pdfwrite', $i));
                eval { GSAPI::exit($gs) };
                $stdout_data = '';
                if (open my $fh, '<:raw', "$scratch") { local $/; $stdout_data = <$fh>; close $fh; }
                eval { GSAPI::delete_instance($gs) };
                $gs = undef;
                $gs_needs_init = 1;
                if (substr($stdout_data, -64) !~ /%%EOF\s*\z/) {
                    info $L->(sprintf('/gs render %d still unfinalised after interpreter close', $i));
                    return json_response({rc => 0, render_error => $i + 1,
                        stderr => 'pdfwrite could not be finalised'}, 200);
                }
            }
        } else {
            ($rrc, $stdout_data, $render_stderr) = _run_ps($L, $final_ps, 1);
            return $$rrc if ref $rrc;
        }
    ...
    if ($pdf2svg && ref $pdf2svg eq 'ARRAY') {
        for my $idx (@$pdf2svg) {
            ...
            debug $L->("Converting render $idx PDF to SVG via $svg_backend");
            my $pdf = decode_base64($renders[$idx]{stdout_b64});
            eval {
                $renders[$idx]{svg} = $svg_backend eq 'mupdf'
                    ? MUPDFAPI::pdf_to_svg($mupdf_ctx, $pdf)
                    : _pdf_to_svg_cairo($pdf);
            };
            if ($@) {
